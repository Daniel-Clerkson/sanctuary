  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center opacity-10">
      <svg width="500" height="500" viewBox="0 0 500 500" fill="none">
        ...
        <polygon points="250,80 400,380 100,380" stroke="#C9A84C" strokeWidth="0.5" fill="none" />
        <polygon points="250,420 100,120 400,120" stroke="#C9A84C" strokeWidth="0.5" fill="none" />
      </svg>
    </div>
  )
}
...
function ScriptureCarousel() {
  ...
  return (
    <div className={`max-w-md text-center transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}>
      <blockquote className="font-serif italic text-cream/90 text-xl md:text-2xl leading-relaxed">
        {`"${scriptures[index].text}"`}
      </blockquote>
      <p className="font-mono text-gold/60 text-xs mt-4 tracking-wider">{scriptures[index].ref}</p>
    </div>
  )
}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {mode === 'register' && (
              <>
                ...
                >
                  {departments.map((d) => (
                    <option key={d} value={d} className="bg-surface-elevated text-cream">
                      {d}
                    </option>
                  ))}
                </select>
            ...
            {mode === 'login' && (
              <button type="button" className="font-sans text-xs text-gold self-end hover:text-cream transition-colors">
                Forgot password?
              </button>
            )}
            ...
            <button
              type="submit"
              className="w-full h-12 bg-gold text-[#0A0A0A] font-sans font-medium text-sm rounded-xl hover:scale-[1.02] transition-all duration-200 mt-2"
            >
              {'Continue \u2192'}
            </button>
